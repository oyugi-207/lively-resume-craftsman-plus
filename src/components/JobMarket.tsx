...
const JobMarket: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-4">
      ...
      {/* Saved Jobs Section */}
      {Object.keys(savedJobsData).length > 0 && (
                ...
                {Object.values(savedJobsData).map((job) => (
                  <div key={job.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-800 transition-all group flex items-start justify-between gap-6">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-purple-600 rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md group-hover:shadow-lg">{job.company[0]?.toUpperCase() || 'C'}</div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{job.title}</h3>
                        <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400 mb-2 flex-wrap">
                          ...
                          {job.salary && job.salary !== 'Salary not specified' && (
                            <span className="flex items-center gap-1 text-green-700 dark:text-green-400"><DollarSign className="w-4 h-4" />{job.salary}</span>
                          )}
                        </div>
                        <p className="text-gray-700 dark:text-gray-300 mb-2 leading-relaxed line-clamp-2">{cleanDescription(job.description)}</p>
                        <div className="flex flex-wrap gap-2">
                          ...
                          {job.requirements.slice(0, 3).map((req, reqIndex) => (
                            <Badge key={reqIndex} variant="outline" className="bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-300 dark:border-gray-600 transition-colors">{req}</Badge>
                          ))}
                          {job.requirements.length > 3 && (
                            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700">+{job.requirements.length - 3} more</Badge>
                          )}
                        </div>
                      </div>
                    </div>
